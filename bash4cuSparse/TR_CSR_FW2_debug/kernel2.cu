...
double* SMVP_saveMemory(double* h_C_dense, int* nnz_return, int N);
...
int main(int args, char **argv)
{
	...
	/**************************/
	const int N = 16384;                // --- Number of rows and columns
	...
	// 	char str[64] = { 0 };
		...
		for (int i = 0; i < ginst->vert_count; i++)
		{
			...
			for (int j = beg; j < end; j++)
			{
				...
				h_A_dense[ i *N + ginst->csr[j] ] = 1;
			}
			// std::cout << "\n";
			// cout << "Normal at line 160!!! ";
		}
		cout<<"ginst->vert_count"<< " "<< "ginst->beg_pos[ginst->vert_count]"<<endl;
	cout<<ginst->vert_count<< " "<< ginst->beg_pos[ginst->vert_count]<<endl;
	// 	for (int i = 0; i < words.size(); i++)
	// 	{
	...
	// 		h_A_dense[(int)((arr1[i]) * N + arr2[i])] = 1; ///(arr1[i]-1) don't use it !
	// 	}
	...
	{
		int temp = nnz_return;
		h_A_dense = SMVP_saveMemory(h_A_dense, &nnz_return, N);
		// cout << "nnz= " << nnz_return<<" "<< i << " round\n";
		cout << "-----------edges find = " << nnz_return-ginst->vert_count << " \n";
		// cudaEventRecord(stop, 0);
		// 	cudaEventSynchronize(stop);
		// 	cudaEventElapsedTime(&elapsedTime, start, stop);
			// std::cout << "CUDA spent： " << elapsedTime <<" ms"<< std::endl;
			my_timer.stop();
			std::cout << "my_timer() time use(ms):         : " << my_timer.elapsed()<<endl;
	...
	// cudaEventSynchronize(stop);
	// cudaEventElapsedTime(&elapsedTime, start, stop);
	// std::cout << "CUDA spent： " << elapsedTime << std::endl;
	my_timer.stop();
	std::cout << "my_timer() time use(ms):         : " << my_timer.elapsed();
	exit(0);

	double* h_B_dense;// debug-test
...
/*	double* d_C_dense; */ gpuErrchk(cudaMalloc(&d_C_dense, N * N * sizeof(*d_C_dense)));
	...
	cout << "use function\n\n ";
	 ...
	 nnz_return = 0;
	h_A_dense = SMVP(h_A_dense, &nnz_return, N);

	h_A_dense = SMVP(h_A_dense, &nnz_return, N);
	h_A_dense = SMVP(h_A_dense, &nnz_return, N);
	h_A_dense = SMVP(h_A_dense, &nnz_return, N);    return 0;  
